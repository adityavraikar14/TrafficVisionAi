        ...
        <div className="flex flex-col gap-5">
            ...
            {cities.length === 0 ? (
              ...
              <>
                <div className="text-center">
                  ...
                  <div className="text-[26px] font-black text-tv-text mt-2">{cities[0].city} – {cities[0].total} Violations</div>
                  ...
                  {cities[0].last_violation_at && (
                    <div className="text-tv-muted text-xs mt-1">
                      Last violation: {new Date(cities[0].last_violation_at).toLocaleString()}
                    </div>
                  )}
                </div>
          ...
          <Card title="Risk Legend">
            {[
              ...
              <div key={l.label} className="flex justify-between items-center py-2.5 border-b border-tv-border last:border-0">
                <div>
                  <div className="font-extrabold text-tv-text text-sm">{l.label}</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-tv-muted text-[11px] uppercase tracking-wide border-b border-tv-border">
                ...
                <th className="py-2 pr-4">Last Violation</th>
              </tr>
            </thead>
            <tbody>
              {cities.map((c) => (
                <tr key={c.city} className="border-b border-tv-border">
                  <td className="py-2.5 pr-4 font-bold text-tv-text">{c.city}</td>
                  ...
                  <td className="py-2.5 pr-4 text-tv-muted">{c.last_violation_at ? new Date(c.last_violation_at).toLocaleDateString() : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
